...
def create_order(new_order: CreateOrderEntity, db: Session = Depends(get_db), producer: Producer = Depends(get_kafka_producer)):
...
def edit_order(order_id: int, updated_order: CreateOrderEntity, db: Session = Depends(get_db), producer: Producer = Depends(get_kafka_producer)):
