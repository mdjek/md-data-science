...
from entities import ResponseTaskEntity
...
@router.get("/tasks", response_model=ResponseTaskEntity, dependencies=[Depends(get_current_client)])
def get_tasks(newTask: ResponseTaskEntity):
...
@router.post("/tasks", response_model=ResponseTaskEntity, dependencies=[Depends(get_current_client)])
def create_task(newTask: ResponseTaskEntity):
...
@router.put("/tasks/{task_id}", response_model=ResponseTaskEntity, dependencies=[Depends(get_current_client)])
def edit_task(task_id: str, updated_task: ResponseTaskEntity):
...
@router.get("/orders/{order_id}/tasks", response_model=ResponseTaskEntity, dependencies=[Depends(get_current_client)])
